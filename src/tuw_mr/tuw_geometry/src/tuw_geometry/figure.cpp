...
#include <ros/ros.h> // added by adrian
...
void Figure::init () {
    ...
    //ROS_INFO("width_pixel: %d ", width_pixel_);
    //ROS_INFO("min_x: %f ", min_x_);
    //ROS_INFO("height_pixel_: %d ", height_pixel_);
    //ROS_INFO("max_y_: %f ", max_y_);
    //ROS_INFO("rotation_: %f ", rotation_);
    
    dx_ = 0; //width_pixel_/max_x_;  // visual width
    dy_ = 0; //height_pixel_/max_y_; // visual height
    sx_ = width_pixel_/(max_x_ - min_x_);  // scaling x
    sy_ = height_pixel_/(max_y_ - min_y_);  // scaling y
    ox_ = width_pixel_/2;  // offset image space x
    oy_ = height_pixel_/2; // offset image space y
    mx_ = -0.5*(max_x_+min_x_);  // visual image space x offset
    my_ = -0.5*(max_y_+min_y_);;  // visual image space y offset
    ...
    cv::Matx<double, 3, 3 > R  ( cos(rotation_+M_PI/2), sin(rotation_+M_PI/2), 0, 		-sin(rotation_+M_PI/2), cos(rotation_+M_PI/2), 0, 	0, 0, 1 ); // rotation
